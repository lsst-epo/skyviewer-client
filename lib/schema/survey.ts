...
export const surveyLayerSchema = z
  .object({
    ...
    optionalLayer: z.boolean().default(true).catch(true),
  ...
  .transform(({ id, opacity, showOnLoad, survey, optionalLayer }) => {
    return { id, survey: { opacity, optionalLayer, showOnLoad, ...survey } };
  });
